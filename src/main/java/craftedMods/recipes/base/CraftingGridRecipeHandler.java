...
public abstract class CraftingGridRecipeHandler extends AbstractRecipeHandler<AbstractRecipe> {
	...
	public static final int[][] DEFAULT_SHAPELESS_STACKORDER = new int[][] { { 0, 0 }, { 1, 0 }, { 0, 1 }, { 1, 1 },
			{ 0, 2 }, { 1, 2 }, { 2, 0 }, { 2, 1 }, { 2, 2 } };
	...
	protected void undefinedRecipeTypeFound(IRecipe recipe, Collection<AbstractRecipe> container) {
		if (logUndefinedRecipeTypes) {
			this.logger.warn("The recipe handler \"" + this.getUnlocalizedName() + "\" got a recipe (\""
					+ recipe.getClass() + "\") which couldn't be processed");
		}
	}
