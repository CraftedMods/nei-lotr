...
public class HobbitOvenRecipeHandler extends AbstractRecipeHandler<HobbitOvenRecipe>
{
    ...
    public List<RecipeItemSlot> getSlotsForRecipeItems (HobbitOvenRecipe recipe, EnumRecipeItemRole role)
    {
        ...
        switch (role)
        {
            case INGREDIENT:
                for (int i = 0; i < 9; i++)
                {
                    slots.add (createRecipeItemSlot (3 + i * 18, 10));
                }
                break;
            case RESULT:
                for (int i = 0; i < 9; i++)
                {
                    slots.add (createRecipeItemSlot (3 + i * 18, 56));
                }
                break;
            case OTHER:
                slots.add (createRecipeItemSlot (75, 100));
                break;
        }
        return slots;
    }
    ...
    public class HobbitOvenRecipe extends FurnaceRecipe
    {
        ...
        public HobbitOvenRecipe (ItemStack ingredient, ItemStack result)
        {
            ...
            this.addAll (Arrays.asList (ingredient, ingredient, ingredient, ingredient, ingredient, ingredient,
                ingredient, ingredient), ingredients);
            this.addAll (Arrays.asList (result, result, result, result, result, result, result, result), results);
        }
        ...
        implements RecipeHandlerRenderer<HobbitOvenRecipeHandler, HobbitOvenRecipe>
    {
        ...
        public void renderBackground (HobbitOvenRecipeHandler handler, HobbitOvenRecipe recipe, int cycleticks)
        {
            ...
            RecipeHandlerRendererUtils.getInstance ().drawProgressBar (75, 29, 176, 14, 24, 25, cycleticks % 48 / 48.0f,
                EnumProgressBarDirection.INCREASE_DOWN);
            RecipeHandlerRendererUtils.getInstance ().drawProgressBar (75, 83, 176, 0, 14, 13, cycleticks % 48 / 48.0f,
        ...
        extends AbstractRecipeViewer<HobbitOvenRecipe, HobbitOvenRecipeHandler>
    {
        ...
        public HobbitOvenRecipeHandlerRecipeViewer (HobbitOvenRecipeHandler handler)
        {
            ...
            supportedGuiClasses.add (LOTRGuiHobbitOven.class);
        }
