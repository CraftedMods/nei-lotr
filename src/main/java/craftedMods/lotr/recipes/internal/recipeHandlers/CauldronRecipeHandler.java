...
public class CauldronRecipeHandler extends AbstractRecipeHandler<ShapelessRecipe>
{
    ...
    public Collection<ShapelessRecipe> getDynamicCraftingRecipes (ItemStack result)
    {
        ...
        this.handleUndyedResultItem (LOTRItemHobbitPipe.class, result,
            stack -> LOTRItemHobbitPipe.getSmokeColor (stack) != 0,
            getHobbitPipeDyePermutations (result), recipes);
        this.handleUndyedResultItem (LOTRItemLeatherHat.class, result,
            ((Predicate<ItemStack>) LOTRItemLeatherHat::isHatDyed).or (LOTRItemLeatherHat::isFeatherDyed),
            getLeatherHatDyePermutations (result),
            recipes);
        ...
        if (result.getItem () instanceof ItemArmor && recipes.isEmpty ())
        {
            ...
            {
                this.handleUndyedResultItem (ItemArmor.class, result, armorItem::hasColor,
                    this.getDefaultColorPermutations (result, armorItem::func_82813_b),
                    recipes);
            }
        }
    ...
    private <T extends Item> void handleUndyedResultItem (Class<T> itemType, ItemStack result,
        Predicate<ItemStack> isDyedFunction,
    ...
    private Function<ItemStack, Collection<ItemStack>> getDefaultColorPermutations (ItemStack result,
        BiConsumer<ItemStack, Integer> dyeFunction)
    {
    ...
    private Function<ItemStack, Collection<ItemStack>> getLeatherHatDyePermutations (ItemStack result)
    {
        return this.getDefaultColorPermutations (result, (stack, colorInt) ->
        {
            ...
            LOTRItemLeatherHat.setHatColor (stack,
                new Color (color.getGreen (), color.getRed (), color.getBlue ()).getRGB ());
        });
    }
    ...
    public Collection<ShapelessRecipe> getDynamicUsageRecipes (ItemStack ingredient)
    {
        ...
        this.handleDyeableIngredientItem (LOTRItemPouch.class, ingredient, LOTRItemPouch::isPouchDyed,
            LOTRItemPouch::removePouchDye, recipes);
        this.handleDyeableIngredientItem (LOTRItemHobbitPipe.class, ingredient,
            stack -> LOTRItemHobbitPipe.getSmokeColor (stack) != 0,
            LOTRItemHobbitPipe::removePipeDye, recipes);
        this.handleDyeableIngredientItem (LOTRItemLeatherHat.class, ingredient,
            ((Predicate<ItemStack>) LOTRItemLeatherHat::isHatDyed).or (LOTRItemLeatherHat::isFeatherDyed),
            LOTRItemLeatherHat::removeHatAndFeatherDye,
            recipes);
        this.handleDyeableIngredientItem (LOTRItemFeatherDyed.class, ingredient, LOTRItemFeatherDyed::isFeatherDyed,
            LOTRItemFeatherDyed::removeFeatherDye,
            recipes);
        this.handleDyeableIngredientItem (LOTRItemHaradRobes.class, ingredient, LOTRItemHaradRobes::areRobesDyed,
            LOTRItemHaradRobes::removeRobeDye, recipes);
        this.handleDyeableIngredientItem (LOTRItemPartyHat.class, ingredient, LOTRItemPartyHat::isHatDyed,
            LOTRItemPartyHat::removeHatDye, recipes);
        if (ingredient.getItem () instanceof ItemArmor)
        {
            ItemArmor armorItem = (ItemArmor) ingredient.getItem ();
            this.handleDyeableIngredientItem (ItemArmor.class, ingredient, armorItem::hasColor, armorItem::removeColor,
                recipes);
    ...
    private <T extends Item> void handleDyeableIngredientItem (Class<T> itemType, ItemStack ingredient,
        Predicate<ItemStack> isDyedFunction,
